        ...
        while (have_posts()) {
              ...
              <section class="content">
                ...
                <div class="sticky-nav-project">
                  ...
                  <p>Click <br>to <br>scroll up!</p>
                </div>
                <div class="project-indicator">
                  <img src="/wp-content/themes/seanyb/img/svg/sidearrow.svg"/>
                  <p>Click <br>to <br>view website!</p>
                </div>
                <!--project information-->
                ...
                <div class="project-video">
                 ...
                 //Get the images ids from the post_metadata
                    $images = acf_photo_gallery('video', $post->ID);
                    ...
                    if( count($images) ):
                        ...
                        foreach($images as $image):
                            ...
                            $full_image_url = acf_photo_gallery_resize_image($full_image_url, 960, 642); //Resized size to 262px width by 160px height image url
                ...
                <div class="project-process">
                   ...
                   <?php 
                   //Get the images ids from the post_metadata
                      $images = acf_photo_gallery('site_map', $post->ID);
                      //Check if return array has anything in it
                      if( count($images) ):
                          //Cool, we got some data so now let's loop over it
                          foreach($images as $image):
                              ...
                              $caption= $image['caption']; //The caption
                              $full_image_url= $image['full_image_url']; //Full size image url
                              $full_image_url = acf_photo_gallery_resize_image($full_image_url, 984, 642); //Resized size to 262px width by 160px height image url
                              $thumbnail_image_url= $image['thumbnail_image_url']; //Get the thumbnail size image url 150px by 150px
                              $url= $image['url']; //Goto any link when clicked
                              $target= $image['target']; //Open normal or new tab
                              $alt = get_field('photo_gallery_alt', $id); //Get the alt which is a extra field (See below how to add extra fields)
                 ...
                 //Get the images ids from the post_metadata
                    ...
                    if( count($images) ):
                        ...
                        foreach($images as $image):
                            ...
                            $caption= $image['caption']; //The caption
                            $full_image_url= $image['full_image_url']; //Full size image url
                            $full_image_url = acf_photo_gallery_resize_image($full_image_url, 333, 1024); //Resized size to 262px width by 160px height image url
                            $thumbnail_image_url= $image['thumbnail_image_url']; //Get the thumbnail size image url 150px by 150px
                            $url= $image['url']; //Goto any link when clicked
                            $target= $image['target']; //Open normal or new tab
                            $alt = get_field('photo_gallery_alt', $id); //Get the alt which is a extra field (See below how to add extra fields)
                 ...
                 //Get the images ids from the post_metadata
                    ...
                    if( count($images) ):
                        ...
                        foreach($images as $image):
                            ...
                            $caption= $image['caption']; //The caption
                            $full_image_url= $image['full_image_url']; //Full size image url
                            $full_image_url = acf_photo_gallery_resize_image($full_image_url, 1141.5, 642); //Resized size to 262px width by 160px height image url
                            $thumbnail_image_url= $image['thumbnail_image_url']; //Get the thumbnail size image url 150px by 150px
                            $url= $image['url']; //Goto any link when clicked
                            $target= $image['target']; //Open normal or new tab
                            $alt = get_field('photo_gallery_alt', $id); //Get the alt which is a extra field (See below how to add extra fields)
